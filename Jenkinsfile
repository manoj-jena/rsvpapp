...
"""
  ...
  environment {
      IMAGE_REPO = "manoj-jena/rsvp"
      // Instead of DOCKERHUB_USER, use your Dockerhub name
  }
  stages {
    stage('Build') {
      environment {
        DOCKERHUB_CREDS = credentials('manoj-dockerhub')
      }
      steps {
        container('docker') {
    ...
    stage('Deploy') {
      environment {
        GIT_CREDS = credentials('githubaccess')
        HELM_GIT_REPO_URL = "https://github.com/manoj-jena/rsvpapp-helm-cicd.git"
        GIT_REPO_EMAIL = 'manoj.jena8118@gmail.com'
